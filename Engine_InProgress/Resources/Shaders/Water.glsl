...
void main(void) {
    ...
    vec3 grass_color = mix(vec3(0.0039, 0.5255, 0.5529), vec3(0.0196, 0.3765, 0.4941), v_1);

    // float s1 = step(0.1, v_1);
    // float s2 = step(0.15, v_1);
    // float s3 = step(0.25, v_1);
    // float s4 = step(0.35, v_1);
    // grass_color = mix(grass_color, vec3(0.1922, 0.5608, 0.0471), s1);
    // grass_color = mix(grass_color, vec3(0.2392, 0.6157, 0.0196), s2);
    // grass_color = mix(grass_color, vec3(0.0078, 0.4902, 0.0353), s3);
    // grass_color = mix(grass_color, vec3(0.2314, 0.4706, 0.0078), s4);

    // grass_color = mix(grass_color, grass_color * 0.7, cpow(p_1, 0.9));
    // grass_color = mix(grass_color, grass_color * vec3(0.702, 1.0, 0.0), cpow(p_1, 1.1));

    gl_FragColor = vec4(grass_color, 1.0);
